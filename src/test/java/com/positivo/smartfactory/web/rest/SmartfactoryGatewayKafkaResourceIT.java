...
import com.positivo.smartfactory.config.EmbeddedKafka;
import com.positivo.smartfactory.config.KafkaSseConsumer;
import com.positivo.smartfactory.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SmartfactoryGatewayKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/smartfactory-gateway-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/smartfactory-gateway-kafka/consume")
