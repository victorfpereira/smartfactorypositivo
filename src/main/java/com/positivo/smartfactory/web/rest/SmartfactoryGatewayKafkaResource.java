...
@RestController
@RequestMapping("/api/smartfactory-gateway-kafka")
public class SmartfactoryGatewayKafkaResource {

    private final Logger log = LoggerFactory.getLogger(SmartfactoryGatewayKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public SmartfactoryGatewayKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
